...
class CPlusPlusLspTransformerSourceGenerator(CPlusPlusFileGenerator):
  ...
  def generate_code(self: Self) -> None:
    ...
    self.newline()
    self.write('#include <lsp/specification.h>')
    self.write('#include <lsp/lsp_exception.h>')
...
class CPlusPlusLspLanguageServerSourceGenerator(CPlusPlusFileGenerator):
  ...
  def generate_serve(self: Self) -> None:
    ...
    with self.indent():
      ...
      with self.indent():
        ...
        with self.indent():
          self.write('const rapidjson::Value &idValue = document["id"];')
          self.write('if (idValue.IsString()) {')
          with self.indent():
            self.write('response.id = idValue.GetString();')
          self.write('} else if (idValue.IsInt()) {')
          with self.indent():
        ...
        self.write('}')
        self.newline()
        self.write('if (document.HasMember("method")) {')
        with self.indent():
          self.write('const std::string &method = document["method"].GetString();')
          ...
          with self.indent():
            ...
            self.write('NotificationMessage notification =')
            with self.indent():
              self.write('deserializer.deserializeNotification(document);')
  ...
  def generate_code(self: Self) -> None:
    ...
    self.write('#include <lsp/specification.h>')
    self.write('#include <lsp/lsp_exception.h>')
    self.write('#include <lsp/lsp_language_server.h>')
    self.newline()
    self.write(f'namespace {self.namespace} {{')
    self.newline()
    with self.indent():
